...
const friendlyType = (tags: Record<string, string>): string => {
  for (const t of labelTagOrder) {
    if (tags[t]) return tags[t].replace(/_/g, " ");
  }
  return "place";
};
 ...
 */
export const overpassNearby = async (
  location: LatLng,
  radiusMeters = 100,
  ...
  const out: Array<{ name: string; type: string; vicinity?: string }> = [];
