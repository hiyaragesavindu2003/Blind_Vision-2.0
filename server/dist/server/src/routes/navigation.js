...
import { geocode, getActiveProvider, getWalkingRoute, nearbySearch, } from "../services/routingService.js";
const router = Router();
...
router.post("/route", asyncHandler(async (req, res) => {
    ...
    const route = await getWalkingRoute(parsed.data.origin, destinationCoords, destinationName, parsed.data.language);
    if (!route) {
